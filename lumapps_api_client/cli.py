...
def parse_args():
    ...
    add_arg('api_method', nargs='*', metavar='METHOD_PART',
    ...
    add_arg('--config', '-c', nargs='?', default=None, const=LIST_CONFIGS,
            ...
            metavar='CONF_NAME')
    add_arg('body', nargs='?', type=argparse.FileType('r'),
...
def store_config(api_info, auth_info, conf_name, user=None):
    ...
    conf['configs'][conf_name] = {
        'api': api_info, 'auth': auth_info, 'user': user}
    set_conf(conf)
...
def main():
    ...
    if args.help:
        ...
        return
    params = {p[0]: p[2]
              for p in (a.partition('=') for a in args.api_method if '=' in a)}
    if args.body_file:
