...
def test_discovery_cache_1():
    assert lumapps.api.utils.DiscoveryCache == lumapps.api.utils._DiscoveryCacheSqlite


def test_discovery_cache_dict():
    conn = lumapps.api.utils._get_conn(":memory:")
    with patch("lumapps.api.utils._get_conn", return_value=conn) as _:
...
def test_discovery_cache_sqlite():
    conn = lumapps.api.utils._get_conn(":memory:")
    with patch("lumapps.api.utils._get_conn", return_value=conn) as _:
        c = lumapps.api.utils._DiscoveryCacheSqlite
        assert c.get("foobar.com") is None
        c.set("foobar.com", "bla")
        assert c.get("foobar.com") == "bla"
...
def test_no_sqlite():
    with patch("lumapps.api.utils._get_sqlite_ok", return_value=False):
        c = lumapps.api.utils._DiscoveryCacheSqlite
        assert c.get("foobar.com") is None
        c.set("foobar.com", "bla")
        assert c.get("foobar.com") == "bla"
